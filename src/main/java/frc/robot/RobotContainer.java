  ...
  public void configureBindings() {
    ...
    drivetrain.setDefaultCommand( // Drivetrain will execute this command periodically
        drivetrain.applyRequest(() -> 
          drive.withVelocityX(((xRateLimiter.calculate(-MathUtil.applyDeadband(driveController.getLeftY(), .1)) * MaxSpeed))* .75) // Drive forward with negative Y (forward)
            .withVelocityY(((yRateLimiter.calculate(-MathUtil.applyDeadband(driveController.getLeftX(), .1)) * MaxSpeed))*.75) // Drive left with negative X (left)
    ...
    operatorController.rightTrigger(.1)
        .onTrue(new ParallelRaceGroup(new ShootCmd(1),
